    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .avatar-circle {
            ...
            font-weight: 700;
            font-size: 15px;
            background: linear-gradient(135deg, #2ea043, #1a7431);
            color: white;
        ...
        /* Consecutive messages (no repeat of sender) */
        .bubble-wrap + .bubble-wrap.mine   { margin-top: 1px; }
        .bubble-wrap + .bubble-wrap.theirs { margin-top: 1px; }
...
let currentType   = null; // 'user' | 'group'
let currentId     = null;
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    setInterval(loadUsers, 30000); // refresh list tiap 30 detik
});
...
async function loadUsers() {
    ...
    const gRes = await fetch('/dashboard');
    // Groups dari blade variable
    renderContactList();
}
...
function renderContactList() {
    ...
    if (currentTab === 'users') {
        ...
        // Groups - diambil dari allGroups (populate on tab click)
        if (!allGroups.length) {
...
function renderMessages(msgs) {
    ...
    msgs.forEach((msg, i) => {
        const isGapTop = lastSender !== null && lastSender !== msg.sender_id;
        ...
        const showSender = !mine && currentType === 'group' && isGapTop;

        html += `
        <div class="bubble-wrap ${mine?'mine':'theirs'}${isGapTop?' gap-top':''}">
            ${showSender ? `<div class="bubble-sender">${escHtml(msg.sender_name)}</div>` : ''}
            ${!mine && currentType === 'group' && lastSender !== msg.sender_id
...
function initEcho() {
    ...
    // Listen user status changes
    echo.channel('user-status').listen('UserStatusChanged', (data) => {
        // Update allUsers
        ...
        // Update header jika chat sedang terbuka dengan user ini
        if (currentType === 'user' && currentId === data.user_id) {
            const dot  = document.querySelector('.chat-header .status-dot');
            const stat = document.querySelector('.chat-header-status');
            if (dot)  { dot.className = `status-dot ${data.is_online?'online':'offline'}`; }
            if (stat) {
                stat.textContent = data.is_online ? 'Online' : data.last_seen;
                stat.className   = `chat-header-status ${data.is_online?'is-online':''}`;
            }
        }
    });
}
...
function subscribeChannel() {
    ...
    if (currentType === 'user') {
        const ids = [ME, currentId].sort((a,b)=>a-b);
        currentChannel = `chat.${ids[0]}.${ids[1]}`;
        echo.private(currentChannel).listen('MessageSent', (data) => {
            if (data.sender_id === currentId) {
                appendMessage({ ...data, is_mine: false });
                // Mark read
                fetch('/users'); // refresh unread
            }
        });
    } else {
        currentChannel = `group.${currentId}`;
        echo.channel(currentChannel).listen('MessageSent', (data) => {
...
function openCreateGroup() {
    ...
    memberList.innerHTML = allUsers.map(u => `
...
// Load groups on init from server-side
const INIT_GROUPS = {!! json_encode(auth()->user()->groups()->with('members')->get()->map(fn($g) => [
    'id'      => $g->id,
    'name'    => $g->name,
